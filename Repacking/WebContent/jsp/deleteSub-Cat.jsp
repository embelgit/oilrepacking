...
<meta charset="utf-8">
<script type="text/javascript" src="/Repacking/staticContent/js/subcategory.js"></script>
...
function Back()
	{
		window.location = "subcategoryList.jsp" ;
		 	...
		 	<div class="row header_margin_top">
			    <div align="center">
			  		<h2 class="form-name style_heading"><%if(abc.equals("marathi")){%><%=PropertiesHelper.marathiProperties.getProperty("godownDetails") %> <%}%> <%if(abc.equals("english")){%>Delete Sub-Category<%}%> </h2>
			  	</div>
	...
	<div class="container col-sm-offset-2" >
           		...
           		<label class="col-md-offset-2  col-md-2  control-label" for="godown"><%if(abc.equals("marathi")){%><%=PropertiesHelper.marathiProperties.getProperty("godownName") %> <%}%> <%if(abc.equals("english")){%>Category<%}%><sup>*</sup></label>  
            			...
            			<div class="input-group">
							...
							CategoryDetailsDao dao1 = new CategoryDetailsDao();
           						List unitList =dao1.getAllsubCat();
				...
				<datalist id="proName_drop1">
							<%
					           for(int i=0;i<unitList.size();i++){
					        	   SubCategoryDetailsBean bean =(SubCategoryDetailsBean)unitList.get(i);
							%>
		
							<option data-value="<%=bean.getSubcatId()%>" value="<%=bean.getSubcategoryName()%>">
							<%
				      			}
				    		%>
       ...
       <input type="button"  id="btn" name="btn" style="font-size: 25" class="btn btn-large btn-success glyphicon glyphicon-save  button-height-width"  onclick="delsubCat()" value="Delete">
